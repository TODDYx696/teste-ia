...
renderer.setPixelRatio(isMobile?Math.min(devicePixelRatio||1,1.15):Math.min(devicePixelRatio||1,1.65));
renderer.setSize(innerWidth,innerHeight,false);
renderer.outputColorSpace=THREE.SRGBColorSpace;
renderer.toneMapping=THREE.ACESFilmicToneMapping;
renderer.toneMappingExposure=isMobile?1.05:1.18;

// REAL-TIME OCEAN: Three's Water material uses a reflective render pass and animated normal map.
...
const waterSegments=isMobile?96:180;
...
// Sky dome + physically coherent sun direction.
const sky=new Sky();
...
// Distant volumetric-looking haze layers.
...
// Sun glow particles / sea spray.
const sprayCount=isMobile?180:650;
...
// A distant sailboat makes the scale of the ocean readable.
...
// Aircraft silhouette: intentionally far away, with a soft contrail.
...
ScrollTrigger.create({trigger:'main',start:'top top',end:'bottom bottom',scrub:isMobile?.42:.65,onUpdate:self=>{
  ...
  // Camera begins above the horizon, then skims toward and finally below the surface.
...
gsap.fromTo('#loader',{opacity:1},{opacity:0,duration:.8,delay:.45,onComplete:()=>document.querySelector('#loader')?.remove()});
...
function resize(){const w=innerWidth,h=innerHeight;camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setPixelRatio(isMobile?Math.min(devicePixelRatio||1,1.15):Math.min(devicePixelRatio||1,1.65));renderer.setSize(w,h,false);ScrollTrigger.refresh();}
...
function animate(){
  ...
  // The reflection sun drifts subtly, changing the highlight across the sea.
  ...
  // Boat rocks with the passing swell.
