...
const renderer = new THREE.WebGLRenderer({ canvas, antialias: !isMobile, alpha: true, powerPreference: 'high-performance', failIfMajorPerformanceCaveat: false });
...
const ground = new THREE.Mesh(new THREE.PlaneGeometry(90,120), new THREE.MeshStandardMaterial({ color:0x07140c, roughness:1 }));
...
scene.add(new THREE.Points(particles, new THREE.PointsMaterial({ color:0xb7ff67, size:isMobile?.028:.035, transparent:true, opacity:.65, depthWrite:false })));

// Avião 3D leve: cruza o céu no começo da experiência sem depender de um arquivo externo.
const airplane = new THREE.Group();
const planeScale = isMobile ? .62 : .9;
const bodyMat = new THREE.MeshStandardMaterial({ color:0xe7ece8, roughness:.42, metalness:.18 });
const darkMat = new THREE.MeshStandardMaterial({ color:0x26332c, roughness:.6, metalness:.2 });
const fuselage = new THREE.Mesh(new THREE.CylinderGeometry(.16,.2,2.8,12), bodyMat);
fuselage.rotation.z = Math.PI/2;
airplane.add(fuselage);
const nose = new THREE.Mesh(new THREE.ConeGeometry(.2,.52,12), bodyMat);
nose.rotation.z = -Math.PI/2;
nose.position.x = 1.65;
airplane.add(nose);
const wing = new THREE.Mesh(new THREE.BoxGeometry(1.5,.055,.5), bodyMat);
wing.position.x = .05;
airplane.add(wing);
const tailWing = new THREE.Mesh(new THREE.BoxGeometry(.62,.045,.24), bodyMat);
tailWing.position.set(-1.05,.18,0);
airplane.add(tailWing);
const tailFin = new THREE.Mesh(new THREE.BoxGeometry(.32,.42,.045), darkMat);
tailFin.position.set(-1.05,.25,0);
...
ScrollTrigger.create({ trigger:'main', start:'top top', end:'bottom bottom', scrub:isMobile?.35:.7, onUpdate:self => {
...
gsap.to('.hero-real-image',{ scale:1.16, yPercent:8, scrollTrigger:{ trigger:'.hero', start:'top top', end:'bottom top', scrub:true }});
gsap.utils.toArray('.story-card').forEach(card => gsap.fromTo(card,{y:55,opacity:0},{y:0,opacity:1,scrollTrigger:{trigger:card,start:'top 82%',end:'top 52%',scrub:true}}));
gsap.fromTo('#loader',{opacity:1},{opacity:0,duration:.7,delay:.45,onComplete:()=>document.querySelector('#loader')?.remove()});

if (!isMobile) addEventListener('pointermove',e=>{ state.mouseX=(e.clientX/innerWidth-.5)*2; state.mouseY=(e.clientY/innerHeight-.5)*2; },{passive:true});
...
function animate(){
  ...
  // Voo contínuo, com passagem mais evidente durante a abertura.
  const phase=(t*(isMobile?.65:1))%16/16;
  airplane.position.x=-18+phase*36;
  airplane.position.y=7.2+Math.sin(phase*Math.PI)*.65;
  airplane.position.z=-5-phase*9;
  airplane.rotation.z=-.035+Math.sin(phase*Math.PI*2)*.025;
  airplane.rotation.y=Math.PI/2+Math.sin(phase*Math.PI)*.035;
