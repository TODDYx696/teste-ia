...
const water=new Water(new THREE.PlaneGeometry(waterSize,waterSize,waterSegments,waterSegments),{
  textureWidth:isMobile?128:512,
  textureHeight:isMobile?128:512,
  waterNormals,
  sunDirection:new THREE.Vector3(-0.35,0.85,0.2).normalize(),
  sunColor:0xfff4d0,
  waterColor:0x075a73,
  distortionScale:isMobile?2.6:3.8,
  fog:true
});
...
const hull=new THREE.Mesh(new THREE.BoxGeometry(3.2,.42,.85),new THREE.MeshStandardMaterial({color:0x10242a,roughness:.7}));hull.position.y=.45;boat.add(hull);
const mast=new THREE.Mesh(new THREE.CylinderGeometry(.035,.035,3.2,8),new THREE.MeshStandardMaterial({color:0xf2eee0,roughness:.6}));mast.position.y=2;boat.add(mast);
const sail=new THREE.Mesh(new THREE.PlaneGeometry(1.9,2.2),new THREE.MeshStandardMaterial({color:0xf3f0df,side:THREE.DoubleSide,roughness:.9}));sail.position.set(.55,2.05,0);sail.rotation.y=-.18;boat.add(sail);
...
const wing=new THREE.Mesh(new THREE.BoxGeometry(1.2,.04,.45),planeMat);plane.add(wing);plane.scale.setScalar(isMobile?.72:1);scene.add(plane);
const trailGeo=new THREE.BufferGeometry();const trailPos=new Float32Array(48*3);trailGeo.setAttribute('position',new THREE.BufferAttribute(trailPos,3));
...
function resize(){const w=innerWidth,h=innerHeight;camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setPixelRatio(isMobile?Math.min(devicePixelRatio||1,1.1):Math.min(devicePixelRatio||1,1.65));renderer.setSize(w,h,false);ScrollTrigger.refresh();}
...
function animate(){
  ...
  boat.rotation.z=Math.sin(t*.55)*.025;boat.rotation.x=Math.cos(t*.42)*.018;boat.position.y=Math.sin(t*.55)*.12;
